...
func _ready():
	if !FileAccess.file_exists(SETTINGS_FILE_PATH):
		...
		config.set_value("keybinding", "sword", "X")
		config.set_value("keybinding", "jump", "Space")
		config.set_value("keybinding", "save_game", "F5")
		config.set_value("keybinding", "load_game", "F6")
		config.set_value("keybinding", "delete_game", "F7")

		config.set_value("video", "fullscreen", false)
		config.set_value("video", "borderless", false)
		config.set_value("video", "V-sync", 1)
